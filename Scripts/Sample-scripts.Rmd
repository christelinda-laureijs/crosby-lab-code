...
```{r, warning=FALSE}
table_data <- raw_eEPSC_df %>% 
  group_by(letter, synapses, sex, treatment, X, Y, age, animal, category, cell) %>% 
  summarize(P1_transformed_list = list(P1_transformed)) %>% 
merge(., treatment_names_and_colours, by = "treatment") %>% select(c(letter, display_names, synapses, sex, X, Y, age, animal, category, cell, P1_transformed_list, colours)) %>% 
...
reactable(data = table_data,
    ...
    P1_transformed_list = colDef(name = "P1 transformed",
      cell = react_sparkline(table_data,
