...
save_plot_pngs <- "no" # Default is "no" to decrease run-time
```
...
# TODO create sample data to add

```{r, eval=FALSE}
add_new_cells(new_raw_data_csv = "Data/Raw-CSVs/eEPSC-data/ia-ic-id.csv",
...
```{r, eval=FALSE}
add_new_cells(new_raw_data_csv = "Data/AD-Only.csv",
              cell_characteristics_csv = "Data/Sample-cell-characteristics.csv",
...
eEPSC_individual_raw_plots <- pmap(
...
eEPSC_mean_P1_pruned_plots_PPP_category_2
```
...
sEPSC_amplitude_pruned_plots_PPP_category_2
```
...
# TODO Fix this level sets of factors are different
```{r}
...
```{r}
BN_baseline <- import_ABF_file("23711004.abf")
BN_insulin <- import_ABF_file("23711009.abf")
